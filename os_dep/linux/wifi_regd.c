					   ...
					   enum nl80211_reg_initiator initiator)
{
	...
	for (band = 0; band < WDEV_NUM_BANDS; band++) {
					     ...
					     initiator)
{
	...
	if (!wiphy->bands[WDEV_BAND_DEF_2GHZ])
		return;
	sband = wiphy->bands[WDEV_BAND_DEF_2GHZ];
...
static void _rtw_reg_apply_radar_flags(struct wiphy *wiphy)
{
	...
	if (!wiphy->bands[WDEV_BAND_DEF_5GHZ])
		return;

	sband = wiphy->bands[WDEV_BAND_DEF_5GHZ];
