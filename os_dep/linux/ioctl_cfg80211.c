...
#define NL80211_WAPI_VERSION_1          (1 << 2)
	...
	#define WDEV_BAND_DEF_2GHZ 	NL80211_BAND_2GHZ
	#define WDEV_BAND_DEF_5GHZ 	NL80211_BAND_5GHZ
	#define WDEV_BAND_ENUM		nl80211_band
#else
	#define WDEV_BAND_DEF_2GHZ 	IEEE80211_BAND_2GHZ
	#define WDEV_BAND_DEF_5GHZ 	IEEE80211_BAND_5GHZ
	#define WDEV_BAND_ENUM		ieee80211_band
#endif
...
struct ieee80211_supported_band *rtw_spt_band_alloc(
	enum WDEV_BAND_ENUM band
	)
{
	struct ieee80211_supported_band *spt_band = NULL;
	int n_channels, n_bitrates;

	if(band == WDEV_BAND_DEF_2GHZ)
	{
		n_channels = RTW_2G_CHANNELS_NUM;
		n_bitrates = RTW_G_RATES_NUM;
	}
	else if(band == WDEV_BAND_DEF_5GHZ)
	{
...
void rtw_spt_band_free(struct ieee80211_supported_band *spt_band)
{
	...
	if(spt_band->band == WDEV_BAND_DEF_2GHZ)
	{
		size = sizeof(struct ieee80211_supported_band)
			+ sizeof(struct ieee80211_channel)*RTW_2G_CHANNELS_NUM
			+ sizeof(struct ieee80211_rate)*RTW_G_RATES_NUM;
	}
	else if(spt_band->band == WDEV_BAND_DEF_5GHZ)
	{
		size = sizeof(struct ieee80211_supported_band)
			+ sizeof(struct ieee80211_channel)*RTW_5G_CHANNELS_NUM
			+ sizeof(struct ieee80211_rate)*RTW_A_RATES_NUM;		
...
static int rtw_ieee80211_channel_to_frequency(int chan, int band)
{
	...
	* there are overlapping channel numbers in 5GHz and 2GHz bands */
	...
	if (band == WDEV_BAND_DEF_5GHZ) {
       	if (chan >= 182 && chan <= 196)
			return 4000 + chan * 5;
             else
                    return 5000 + chan * 5;
       } else { /* WDEV_BAND_DEF_2GHZ */
		if (chan == 14)
			return 2484;
             else if (chan < 14)
			return 2407 + chan * 5;
...
struct cfg80211_bss *rtw_cfg80211_inform_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
			...
			if(request->n_ssids == 1 && request->n_channels == 1) // it means under processing WPS
			{
				...
				else
				{
					...
					if (pselect_network->Configuration.DSConfig <= RTW_CH_MAX_2G_CHANNEL)
						freq = rtw_ieee80211_channel_to_frequency(pselect_network->Configuration.DSConfig, WDEV_BAND_DEF_2GHZ);
					else
						freq = rtw_ieee80211_channel_to_frequency(pselect_network->Configuration.DSConfig, WDEV_BAND_DEF_5GHZ);
	...
	if (channel <= RTW_CH_MAX_2G_CHANNEL)
		freq = rtw_ieee80211_channel_to_frequency(channel, WDEV_BAND_DEF_2GHZ);
	else
		freq = rtw_ieee80211_channel_to_frequency(channel, WDEV_BAND_DEF_5GHZ);
...
int rtw_cfg80211_check_bss(_adapter *padapter)
{
	...
	if (pnetwork->Configuration.DSConfig <= RTW_CH_MAX_2G_CHANNEL)
		freq = rtw_ieee80211_channel_to_frequency(pnetwork->Configuration.DSConfig, WDEV_BAND_DEF_2GHZ);
	else
		freq = rtw_ieee80211_channel_to_frequency(pnetwork->Configuration.DSConfig, WDEV_BAND_DEF_5GHZ);
...
void rtw_cfg80211_ibss_indicate_connect(_adapter *padapter)
{
	...
	if (cur_network->network.Configuration.DSConfig <= RTW_CH_MAX_2G_CHANNEL)
		freq = rtw_ieee80211_channel_to_frequency(cur_network->network.Configuration.DSConfig, WDEV_BAND_DEF_2GHZ);
	else
		freq = rtw_ieee80211_channel_to_frequency(cur_network->network.Configuration.DSConfig, WDEV_BAND_DEF_5GHZ);
...
void rtw_cfg80211_indicate_disconnect(_adapter *padapter)
{
	...
	if (!padapter->mlmepriv.not_indic_disco) {
		...
		if(check_fwstate(&padapter->mlmepriv, _FW_LINKED))
			#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,11,0)
			//#TODO: Should locally_requested be _TRUE???
			cfg80211_disconnected(padapter->pnetdev, 0, NULL, 0, _FALSE, GFP_ATOMIC);
			#else
			cfg80211_disconnected(padapter->pnetdev, 0, NULL, 0, GFP_ATOMIC);
			#endif
		else
...
void rtw_cfg80211_indicate_scan_done(_adapter *adapter, bool aborted)
{
	...
	if (pwdev_priv->scan_request != NULL) {
		...
		{
			#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,0,0)
			scan_info.scan_start_tsf = 0;
			for (i = 0; i != ETH_ALEN; ++i)
				scan_info.tsf_bssid[i] = 0;
...
void rtw_cfg80211_indicate_sta_assoc(_adapter *padapter, u8 *pmgmt_frame, uint frame_len)
{
...
#if defined(RTW_USE_CFG80211_STA_EVENT) || defined(COMPAT_KERNEL_RELEASE)
	{
		...
		sinfo.filled = 0;
		#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,0,0)
		//sinfo.filled = BIT(NL80211_STA_INFO_ASSOC_REQ_IES);
		#else
...
static void rtw_cfg80211_init_ht_capab(struct ieee80211_sta_ht_cap *ht_cap, enum WDEV_BAND_ENUM band, u8 rf_type)
{
...
#define MAX_BIT_RATE_40MHZ_MCS7 	150	/* Mbps */
	...
	/*
	 *hw->wiphy->bands[WDEV_BAND_DEF_2GHZ]
	 *base on ant_num
	 *rx_mask: RX mask
...
void rtw_cfg80211_init_wiphy(_adapter *padapter)
{
	...
	{
		bands = wiphy->bands[WDEV_BAND_DEF_2GHZ];
		if(bands)
			rtw_cfg80211_init_ht_capab(&bands->ht_cap, WDEV_BAND_DEF_2GHZ, rf_type);
	}

	if (padapter->registrypriv.wireless_mode & WIRELESS_11A)
	{
		bands = wiphy->bands[WDEV_BAND_DEF_5GHZ];
		if(bands)
			rtw_cfg80211_init_ht_capab(&bands->ht_cap, WDEV_BAND_DEF_5GHZ, rf_type);
	}
...
static void rtw_cfg80211_preinit_wiphy(_adapter *padapter, struct wiphy *wiphy)
{
	...
	*/
	...
	/* if (padapter->registrypriv.wireless_mode & WIRELESS_11G) */
		wiphy->bands[WDEV_BAND_DEF_2GHZ] = rtw_spt_band_alloc(WDEV_BAND_DEF_2GHZ);
#ifdef CONFIG_BAND_5GHZ
	/* if (padapter->registrypriv.wireless_mode & WIRELESS_11A) */
		wiphy->bands[WDEV_BAND_DEF_5GHZ] = rtw_spt_band_alloc(WDEV_BAND_DEF_5GHZ);
#endif
...
void rtw_wdev_free(struct wireless_dev *wdev)
{
	...
	rtw_spt_band_free(wdev->wiphy->bands[WDEV_BAND_DEF_2GHZ]);
	rtw_spt_band_free(wdev->wiphy->bands[WDEV_BAND_DEF_5GHZ]);
